...
import { getUsersStagesRange, getStagesCountByUsers } from 'services/supabaseStages';
...
export const UsersStageCard = () => {
  ...
  useEffect(() => {
    const fetchStagesData = async () => {
      try {
        ...
        const response = await getUsersStagesRange({ start: startPage, end: startPage + STAGE_PER_PAGE - 1 });
  ...
  useEffect(() => {
    const fetchCountStagesData = async () => {
      try {
        const response = await getStagesCountByUsers();
      ...
      <div className="grid grid-cols-2 gap-4">
        {stages.map(({ id, userName, title, image }) => (
          <div key={id} className="relative m-2 p-6 border border-gray-300 shadow-lg rounded-md bg-white">
            ...
            <p className="absolute top-16 left-4 text-3xl font-bold">User: {userName}</p>
            <p className="absolute top-28 left-4 text-2xl font-bold">Title: {title}</p>
            <Link
